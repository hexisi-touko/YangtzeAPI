...
func Login(c *gin.Context) {
	...
	if err != nil {
		...
		case errors.Is(err, model.ErrUserDisabled):
			writeApplicationLoginError(c, &user)
		default:
...
func Register(c *gin.Context) {
	...
	user := model.User{
		...
		AffCode:          strings.TrimSpace(request.AffCode),
	}
	if user.Username == "" {
		common.ApiErrorI18n(c, i18n.MsgInvalidParams)
		return
	...
	application, err := model.CreateUserApplication(&cleanUser, inviterId, reason)
	if err != nil {
		if errors.Is(err, model.ErrEmailAlreadyTaken) {
			common.ApiErrorI18n(c, i18n.MsgUserEmailAlreadyTaken)
			return
	...
	common.ApiSuccessI18n(c, i18n.MsgUserApplicationSubmitted, gin.H{
